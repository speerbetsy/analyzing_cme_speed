...
##  subplt_velocity.plot(tv/60, vy/1000, '+', label='Raw Data')
    ...
    try:
        ...
        # Velocity, Acceleration, and Height are constant and taken from other fits 
...
def histogram(data, desc, bin_min, bin_max, direc):
    ...
    print("this is the largest bin: ", peak_val)
    ...
    plt.legend(loc=2)
    #plt.show()
    plt.savefig("cme_pkls/final_oscillation/" + direc +".png")
    plt.clf()
    return (peak_val)
...
def to_pkl_file(ran, lin_fit_array, quad_fit_array, oscil_fit_array, desc):
    ...
    COLUMNS = ('LIN-FIT', 'QUAD-FIT', 'OSCIL-FIT-AMP','OSCIL-FIT-PER','OSCIL-FIT-PHASE','OSCIL-FIT-VEL','OSCIL-FIT-ACC','OSCIL-FIT-CHISQ')
    ...
    for n in ran:
        df.loc[n] = pd.Series({'LIN-FIT': lin_fit_array[n],
                            ...
                            'OSCIL-FIT-CHISQ':oscil_fit_array[n][6]})
    df.to_pickle('cme_pkls/pkl_files/'+desc+'new'+'.pkl')
    
    
def to_pkl_file2(ran, date_array, amp_array, per_array, phase_array, vel_array, acc_array, chi_array, quad_chi_array):
     # Save fit and rchi values to .pkl file
    index = np.arange(0,ran)
    COLUMNS = ('DATE', 'AMP','PER','PHASE','VEL','ACC', 'OSCIL-CHI','QUAD-CHI')
    df = pd.DataFrame(columns=COLUMNS, index=index)
    for n in index:
        df.loc[n] = pd.Series({'DATE': date_array[n],
                            'AMP': amp_array[n],
                            'PER': per_array[n],
                            'PHASE': phase_array[n],
                            'VEL': vel_array[n],
                            'ACC':acc_array[n],
                            'OSCIL-CHI':chi_array[n],
                            'QUAD-CHI':quad_chi_array[n]} )
    df.to_pickle('cme_pkls/final_oscillation/final_oscil_vals_new.pkl')
